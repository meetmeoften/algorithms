	...
	public static void wallsAndGates(int[][] rooms) {
		...
		while (!q.isEmpty()) {
			...
			for (int[] dir : dirs) {
				...
				// So, we can just grab that value (rooms[row][col]) and add 1 to it and change
				// it every time
				rooms[x][y] = rooms[row][col] + 1;
				// So, one level further from door (value 0) is equal to 1. Now, we do bfs from
				// that position so value will be 2 and so on.
			}
		}
	}
