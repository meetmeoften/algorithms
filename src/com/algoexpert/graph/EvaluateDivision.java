...
public class EvaluateDivision {
	...
	public double[] calcEquation(List<List<String>> equations, double[] values, List<List<String>> queries) {
		...
		for (int i = 0; i < queries.size(); i++) {
			...
			if (!graph.containsKey(divider) || !graph.containsKey(divisor)) {
				continue;
			}
	...
	public static void main(String[] args) {
		...
		for (String[] rows : equations) {
			equationsList.add(Arrays.stream(rows).collect(Collectors.toList()));
		}

		List<List<String>> queriesList = new ArrayList<>();
		for (String[] rows : queries) {
			queriesList.add(Arrays.stream(rows).collect(Collectors.toList()));
		}

		EvaluateDivision division = new EvaluateDivision();
		division.calcEquation(equationsList, values, queriesList);
	}
